...
def DlaNet(
    num_layers=34,
    heads={"hm": 1, "ab": 2, "ang": 1, "reg": 2, "mask": 1},
    head_conv=256,
):
    model = Creat_DlaNet(
...
class Creat_MyNet(lightning.LightningModule):
    def __init__(
        self,
        base_name,
        heads,
...
def main():
    ...
    # 初始化模型
    # model = DlaNet(num_layers=num_layers, heads=heads, head_conv=head_conv)
    model = MyNet(num_layers=num_layers, heads=heads, head_conv=head_conv)

    # 创建虚拟输入数据
    batch_size = 32
    input_channels = 3  # 假设输入是RGB图像
